...
# OFAC Civil Penalties Database (/enforcement/) — hub, 24 year pages and 233
# per-action pages generated by scripts/build_enforcement.py, plus the urls.txt
# the sitemap reads. Same trap as /data/ below: the pages render fine locally
# from the repo, so a missing COPY here shows up only as 404s in production.
